...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        XCTAssertEqual(loader.loadFeedCallCount, 1, "Expected no new loading request since previous request hasn't finished yet")
        
        loader.completeFeedLoading(at: 0)
        sut.simulateUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 2, "Expected another loading request once user initiates a reload and previous request finished")
        
        loader.completeFeedLoading(at: 1)
        sut.simulateUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected yet another loading request once user initiates another reload and previous request finished")
    }
    ...
    func test_loadMoreActions_requestMoreFromLoader() {
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 1, "Expected request since load more action has been triggered")
        
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 1, "Expected no request since we are already loading")
        
        loader.completeLoadMore(lastPage: false, at: 0)
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 2, "Expected request after load more completed with new pages")
    ...
    func test_tapOnLoadMoreErrorView_loadMore() {
        ...
        sut.simulateTapOnLoadMoreFeedError() // should have no effect, since no error should be currently displayed
        XCTAssertEqual(loader.loadMoreCallCount, 1)
    ...
    func test_feedImageView_doesNotShowDataFromPreviousRequestWhenCellIsReused() throws {
        ...
        XCTAssertEqual(view0.renderedImage, .none, "Expected no image state change for reused view once image loading (which loads image of previously displayed content) completes successfully")
    }
    
    func test_feedImageView_showsDataForNewViewRequestAfterPreviouViewIsReused() throws {
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageView_doesNotLoadImageAgainUntilPreviousRequestCompletes() {
        ...
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.url], "Expected no new request until previous completes")
        
        loader.completeImageLoading(at: 0)
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.url, image.url], "Expected second request when image becomes visible and previous request finishes")
